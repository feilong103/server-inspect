...
```python
EmailNotifier.send(
    smtp_config: dict,          # SMTP 配置（host, port, user, password, from, to）
    reports: List[ServerReport], # 巡检报告对象列表
    thresholds: dict,           # 告警阈值
    report_path: str,           # 报告文件路径
    signature: str              # 邮件签名
...
**AI 的职责**：
- ✅ 执行巡检脚本 `python3 ~/server-inspect/scripts/run_inspect.py`
- ✅ 读取生成的 Markdown 报告
- ✅ 注入 AI 分析建议（替换 `<!-- AI_SUGGESTIONS -->` 占位符）
- ❌ **不要**自己调用通知函数
- ❌ **不要**自己组织通知内容
- ❌ **不要**幻想数据
