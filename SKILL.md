...
巡检完成后，AI 会询问用户是否需要发送通知：

**支持的通知渠道**：
- 📮 **飞书**：通过飞书机器人 Webhook 发送卡片通知
- 📧 **邮件**：通过 SMTP 发送完整报告

**配置方式**：在 `config.json` 的 `notification` 中配置 Webhook URL 或邮件 SMTP：
...
"notification": {
  ...
  "email": {
    ...
    "smtp_password": "tzwudjpyfnjubccb",
...
> **提示**：AI 会自动查找 skill 目录，无需硬编码路径。数据默认保存在 `~/server-inspect/`（可配置）。
>
> 定时任务可配置**自动发送通知**：在 cron message 中添加"3. 发送飞书/邮件通知（如果配置了）"
